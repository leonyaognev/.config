...
Item {
    ...
    readonly property string colorsJsonPath: {
        let envPath = Quickshell.env("WALLPICKER_COLORS_JSON");
        if (envPath && envPath !== "") return envPath;
        return Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/qs_colors.json";
    }

    Process {
        id: themeReader
        command: ["cat", root.colorsJsonPath]
